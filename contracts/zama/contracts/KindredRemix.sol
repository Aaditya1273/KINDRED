...
// File contracts/ConfidentialAlpha.sol

// Original license: SPDX_License_Identifier: MIT
 


// KINDRED: ConfidentialAlpha (Remix Flattened)
...
contract ConfidentialAlpha {
    // World ID Router (Sepolia)
    address public immutable worldIdRouter = 0x469449f251692E0779667583026b5A1E99512157;
    uint256 public immutable groupId = 1; // Production: 1 for Orb verified
    
    // Mapping to prevent sybil/replay attacks
    ...
    /**
     * @notice Verifies a World ID proof on-chain and gates the agent
     */
    function verifyWorldID(
        ...
        // 1. Ensure nullifier hasn't been used (one human per agent)
        require(!nullifierHashes[nullifierHash], "Human identity already tied to an agent");

        // 2. Call WorldIDRouter to verify proof
