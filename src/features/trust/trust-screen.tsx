    ...
    return (
        ...
        >
            <View style={{ flex: 1 }}>
                ...
                <Text style={styles.holdValue} numberOfLines={1}>
                    {revealed ? value : '••••••••••••••••'}
                </Text>
            </View>
            {revealed ? <Eye size={16} color={Colors.cyan} /> : <EyeOff size={16} color={Colors.textMuted} />}
        </Pressable>
    );
}
...
function ReasoningRow({ action, reason, expanded, onToggle }: {
    ...
    return (
        <Pressable onPress={onToggle} style={styles.reasonRow}>
            <View style={styles.reasonHeader}>
                <CheckCircle size={14} color={Colors.emerald} />
                <Text style={styles.reasonAction}>{action}</Text>
    ...
    return (
        ...
        >
            ...
            <Animated.View entering={FadeInDown.duration(300)}>
                <Card style={styles.fheCard}>
                    <View style={styles.fheHeader}>
                        ...
                        <Text style={styles.fheTitle}>Zama FHE Active</Text>
                        <View style={styles.activeBadge}>
                            <Text style={styles.activeBadgeText}>ON</Text>
                        </View>
                    </View>
                    <Text style={styles.fheDesc}>
            ...
            <Animated.View entering={FadeInDown.delay(80).duration(300)}>
                <Card style={styles.section}>
                    <View style={styles.identityRow}>
                        ...
                        <Text style={styles.sectionTitle}>Identity Verified</Text>
                    </View>
                    ...
                    <View style={styles.verifiedRow}>
                        <CheckCircle size={14} color={Colors.emerald} />
                        <Text style={styles.verifiedText}>World ID — Orb verified</Text>
                    </View>
                    <View style={styles.verifiedRow}>
                        <CheckCircle size={14} color={Colors.emerald} />
                        <Text style={styles.verifiedText}>Lit Protocol PKP — Agent wallet bound</Text>
                    </View>
                    <View style={styles.verifiedRow}>
                        <CheckCircle size={14} color={Colors.emerald} />
                        <Text style={styles.verifiedText}>Storacha CID — Memory immutable</Text>
                    </View>
                    {latestCID !== '' && (
                        <Text style={styles.cidText} numberOfLines={1}>Memory: {latestCID}</Text>
                    )}
                </Card>
            </Animated.View>
            ...
            <Animated.View entering={FadeInDown.delay(160).duration(300)}>
                <Card style={styles.section}>
                    <Text style={styles.sectionTitle}>Why the agent acted</Text>
                    <Text style={styles.sectionDesc}>Tap any action to see the reasoning.</Text>
                    {REASONING.map((r, i) => (
                        <ReasoningRow
                            key={r.action}
                            action={r.action}
...
const styles = StyleSheet.create({
    ...
    fheTitle: { fontSize: 15, fontWeight: '700', color: Colors.textPrimary, flex: 1 },
    activeBadge: { backgroundColor: Colors.cyan + '22', paddingHorizontal: 8, paddingVertical: 2, borderRadius: Radius.full },
    activeBadgeText: { fontSize: 10, color: Colors.cyan, fontWeight: '800' },
    fheDesc: { fontSize: 13, color: Colors.textSecondary, lineHeight: 18, marginBottom: Spacing.md },

    holdRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderTopWidth: 1, borderTopColor: Colors.border },
    holdLabel: { fontSize: 11, color: Colors.textMuted, marginBottom: 2 },
    holdValue: { fontSize: 13, color: Colors.textPrimary, fontWeight: '600' },
    ...
    sectionDesc: { fontSize: 13, color: Colors.textSecondary, lineHeight: 18, marginBottom: Spacing.md },
    ...
    cidText: { fontSize: 10, color: Colors.textMuted, marginTop: Spacing.sm },

    reasonRow: { borderTopWidth: 1, borderTopColor: Colors.border, paddingVertical: Spacing.sm },
    reasonHeader: { flexDirection: 'row', alignItems: 'center', gap: 8 },
    reasonAction: { flex: 1, fontSize: 13, color: Colors.textPrimary, fontWeight: '600' },
